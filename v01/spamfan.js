...
function test(){
Logger.log(matchStrings("Maxim Grebennikov, jobs chosen for you", "jobs", ["respectCase","respectSpaces"]))
...
if(!daysOffset){
 ...
 var direction = after ? "newer_than:" : "older_than:"
...
if(threads && threads.length >= 1 && threads.every(function(thread){return thread.getMessageCount() === 1})){
  ...
  config.daysToKeepEmailsInInbox = config.daysToKeepEmailsInInbox === undefined ? 365 : config.daysToKeepEmailsInInbox;
 ...
 for(var idx in emails){
   ...
   for(var lblIdx in allLabels){
     appLogger("Evaluating against label \"" + allLabels[lblIdx] +  "\" ...")
       ...
       result = evaluateSubject(emails[idx], config.labels[allLabels[lblIdx]].detectsubject, config.labels[allLabels[lblIdx]].excludesubject, config.labels[allLabels[lblIdx]].subjectMatchRules)
       ...
       result = evaluateBody(emails[idx], config.labels[allLabels[lblIdx]].detectbody, config.labels[allLabels[lblIdx]].excludebody, config.labels[allLabels[lblIdx]].bodyMatchRules)
...
return evaluateSender(email ,globalExcludeLabels.excludesender, []) ||
      evaluateSubject(email, globalExcludeLabels.excludesubject, [], globalExcludeLabels.subjectMatchRules) ||
      evaluateBody(email, globalExcludeLabels.excludebody, [], globalExcludeLabels.bodyMatchRules )
...
function matchStrings(string1, string2, rulesArr){
  ...
  var preparedStr1 = string1
  var preparedStr2 = string2
...
appLogger("Evaluating subject line " + subject + " ... respectCase rule set to " + rulesArr.includes(CONSTANTS.MATCHING_RULES.RESPECT_CASE) + ", respectSpaces rule set to " +  rulesArr.includes(CONSTANTS.MATCHING_RULES.RESPECT_SPACES) )
...
var body = email.getPlainBody().split("\n").map(function(line){return line})
 appLogger("Evaluating email body from  " + email.getFrom() +  " ... respectCase rule set to " + rulesArr.includes(CONSTANTS.MATCHING_RULES.RESPECT_CASE) + ", respectSpaces rule set to " +  rulesArr.includes(CONSTANTS.MATCHING_RULES.RESPECT_SPACES))
